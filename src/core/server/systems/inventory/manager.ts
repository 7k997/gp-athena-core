...
export async function toggleItem(player: alt.Player, slot: number, type: InventoryType): Promise<boolean> {
    ...
    if (type === 'inventory') {
        ...
        // Corechange: Not only weapons can be equipped from toolbar. So set isEquipped to false for all items.
        // ONLY one item can be equipped at a time in toolbar. For inventory this is different and multiple items can be equipped.
        ...
        for (let i = 0; i < dataCopy.length; i++) {
            ...
            if (dataCopy[i].data && dataCopy[i].data.hash && dataCopy[i].isEquipped) {
                dataCopy[i].isEquipped = false;
                Athena.player.events.trigger('player-weapon-unequipped', player, dataCopy[i].slot, type);
            } else if (dataCopy[i].isEquipped) {
                dataCopy[i].isEquipped = false;
                Athena.player.events.trigger('item-unequipped', player, dataCopy[i].slot, type);
            }
        }
    }
