...
class InternalSystem {
    ...
    static create(interior: Interior) {
        ...
        if (!interior.removeOutsideColshape) {
            ...
            const outsideUid = Athena.controllers.interaction.append({
    ...
    static async refreshInteriorForPlayer(player: alt.Player) {
        ...
        for (let i = 0; i < objects.length; i++) {
            Athena.controllers.object.removeFromPlayer(player, objects[i].uid);
        }
    ...
    static refreshInteriorText(interior: InteriorInternal): void {
        ...
        Athena.controllers.textLabel.remove(`${interior.uid}-outside`);
    ...
    static async toggleLock(player: alt.Player, uid: string, skipOwnerCheck = false) {
        ...
        if (!interior.isUnlocked && interior.storage) {
            Athena.systems.storage.removeAsOpen(interior.storage);
        }
    ...
    static async storage(player: alt.Player, uid: string) {
        ...
        if (!interior.storage || Array.isArray(interior.storage)) {
            storageID = await Athena.systems.storage.create([]);
        ...
        if (Athena.systems.storage.isOpen(storageID)) {
            return;
        }

        Athena.systems.storage.setAsOpen(storageID);
...
export class InteriorSystem {
    ...
    static async movePlayerIn(
        ...
        if (!interior.insideShape) {
            const insideUid = Athena.controllers.interaction.append({
                description: LOCALE_INTERIOR_VIEW.LABEL_OPEN_INTERIOR_MENU,
    ...
    static async showMenu(player: alt.Player, uid: string, isOutside: boolean = true) {
        ...
        alt.log('ShowInterior Menu: ' + JSON.stringify(interior));
...
Athena.player.events.on('selected-character', InternalSystem.spawn);
