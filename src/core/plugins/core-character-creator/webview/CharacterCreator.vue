...
import { CHARACTER_CREATOR_LOCALE } from '../shared/locale/locale.js';
...
//Set Default language if nothing other set
let defaultLanguage = LocaleController.getDefaultLocale().toString();
...
export default defineComponent({
    ...
    data() {
        return {
            ...
            totalCharacters: 1,
            locales: CHARACTER_CREATOR_LOCALE[defaultLanguage],
        };
    },
    computed: {
        isInactive() {
    ...
    methods: {
        ...
        setReady(noDiscard = true, noName = true, languageName = 'English') {
            ...
            let localeKey = LOCALE[languageName];
            if(LocaleController.isLocaleValid(localeKey) && !!CHARACTER_CREATOR_LOCALE[localeKey] ){
                this.locales = CHARACTER_CREATOR_LOCALE[localeKey];
            }
