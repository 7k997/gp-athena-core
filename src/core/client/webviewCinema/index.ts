export { Page } from '../webview/page.js';
...
const InternalFunctions = {
    ...
    async updatePages() {
        ...
        alt.log(`[Vue] === Updating Cinema Pages`);
    ...
    handleClientEvent(eventName: string, ...args: any[]) {
        ...
        if (!ClientEvents[eventName]) {
            alt.logWarning(`Event ${eventName} called but not registered with WebViewCinemaController.on`);
            return;
        }
    ...
    handleReadyEvent(pageName: string, ...args: any[]) {
        if (!ReadyEvents[pageName]) {
            alt.logWarning(
                `Ready Event for ${pageName} called but not registered with WebViewCinemaController.onReady`,
            );
            return;
        }
 ...
 *
 */
export function create(url: string, model: string) {
    _defaultURL = url;
    ...
    if (!_webview) {
        _webview = new alt.WebView(_defaultURL, alt.hash(model), 'script_rt_cinscreen');
